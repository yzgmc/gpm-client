...
国内加速：默认使用 BMCLAPI 镜像（bangbang93.com），下载速度远优于官方源。
...
def _lib_url(lib: dict) -> str:
    """library -> 下载 URL。优先 downloads.artifact.url，否则拼 maven 路径。"""
    dl = (lib.get("downloads") or {}).get("artifact") or {}
    if dl.get("url"):
        # 官方 URL 含 libraries.minecraft.net，替换为 BMCLAPI 镜像
        url = dl["url"]
        return url.replace("https://libraries.minecraft.net/", _LIBRARIES_BASE)
    # 兜底：拼 maven 路径
    name = lib.get("name", "")
    parts = name.split(":")
    if len(parts) < 3:
    ...
    if classifier:
        filename += f"-{classifier}"
    filename += ".jar"
    return f"{_LIBRARIES_BASE}{group_path}/{artifact}/{version}/{filename}"
...
def _download_many(
    items: list[tuple[str, str, Optional[str]]],  # (url, dest, sha1)
    ...
    def _work(item: tuple[str, str, Optional[str]]) -> None:
        nonlocal done, last_pct
        url, dest, sha1 = item
        _download_one(url, dest, sha1, cancel_event)
        with done_lock:
            done += 1
...
def ensure_vanilla_version(
    ...
    if not os.path.isfile(client_jar_path):
        ...
        client_url = client["url"].replace(
            "https://piston-data.mojang.com", "https://bmclapi2.bangbang93.com"
        ).replace(
            "https://launcher.mojang.com", "https://bmclapi2.bangbang93.com"
        )
        _download_one(client_url, client_jar_path, client.get("sha1"), cancel_event)

    # ---- 3. libraries ----
    all_libs = [l for l in vj.get("libraries", []) if _lib_allowed(l)]
    lib_items: list[tuple[str, str, Optional[str]]] = []
    for lib in all_libs:
    ...
    if asset_index:
        ...
        if not os.path.isfile(index_path):
            ...
            ai_url = asset_index["url"].replace(
                "https://piston-meta.mojang.com", "https://bmclapi2.bangbang93.com"
            ).replace(
                "https://launchermeta.mojang.com", "https://bmclapi2.bangbang93.com"
            )
            os.makedirs(index_dir, exist_ok=True)
            _download_one(ai_url, index_path, asset_index.get("sha1"), cancel_event)

        with open(index_path, "r", encoding="utf-8") as f:
            ai = json.load(f)
        objects = ai.get("objects", {})
        asset_items: list[tuple[str, str, Optional[str]]] = []
        for _name, obj in objects.items():
            ...
            if os.path.isfile(obj_path):
                continue  # 已有，跳过
            url = f"{_ASSETS_BASE}{prefix}/{h}"
            asset_items.append((url, obj_path, h))
        if asset_items:
            if progress:
